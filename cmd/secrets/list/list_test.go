...
func testRunCmdList(owner string, repos []string, cmdFlags *cmdFlags, getter *testAPIGetter, reportWriter io.Writer) error {
	...
	// Process secrets for each repo's environments
	for _, singleRepo := range allRepos {
		...
		for _, env := range envList.Environments {
			envSecretResp, err := getter.GetEnvironmentSecrets(owner, singleRepo.Name, env.Name)
			if err != nil {
				continue
			}
			...
			for _, eSecret := range envSecret.Secrets {
				...
				if !eSecret.CreatedAt.IsZero() {
					createdAt = eSecret.CreatedAt.Format(time.RFC3339)
				}

				if !eSecret.UpdatedAt.IsZero() {
					updatedAt = eSecret.UpdatedAt.Format(time.RFC3339)
				}
				...
				err := csvWriter.Write([]string{
					...
					env.Name,
					eSecret.Name,
					"", // Secret value is not accessible
					createdAt,
					updatedAt,
				})
				if err != nil {
...
func TestRunCmdListWithSpecificRepository(t *testing.T) {
	...
	secretsResponse := data.EnvSecret{
		TotalCount: 2,
		Secrets: []data.Secret{
			{
				Name:      "SECRET_1",
	...
	mockGetter.EnvironmentSecretsData = secretsResponseBytes
	...
	// Check if output contains the secret data
	if !strings.Contains(output, "SECRET_1") {
		t.Error("Output does not contain SECRET_1")
	}

	if !strings.Contains(output, "SECRET_2") {
		t.Error("Output does not contain SECRET_2")
	}
}
